                    ...
                    <div className="flex gap-2 self-end mt-2">
                      <button
                        onClick={async () => {
                          if (
                            !window.confirm(
                              `¿Eliminar el epic con id ${epic.epicId}?`
                            )
                          )
                            return;
                          try {
                            ...
                            const res = await fetch(
                              `/api/tasks/epics/?epicId=${epic.epicId}`,
                              {
                                method: "DELETE",
                                headers: {
                            ...
                            if (!res.ok)
                              throw new Error("No se pudo eliminar el epic");
                            fetchEpics();
                        ...
                        <form
                          ...
                          onSubmit={async (e) => {
                            ...
                            try {
                              ...
                              if (!res.ok)
                                throw new Error("No se pudo editar el epic");
                              setEditEpicId(null);
                              fetchEpics();
                              alert("Epic editado");
                            } catch {
